import { type CompareRow } from "@/lib/iphone-configs";
  ...
  return (
    <section className="mb-12">
      ...
      <p className="text-sm text-muted-foreground mb-5">
        Стоит ли переплачивать за новую модель?
      </p>
      <div className="rounded-2xl border border-border overflow-hidden">
        ...
        {rows.map((row, i) => (
          <div
            key={row.label}
            className={`grid grid-cols-3 gap-4 px-5 py-3.5 text-sm items-center ${
              i % 2 === 0 ? "bg-muted/30" : "bg-card"
            }`}
          >
            <span className="text-muted-foreground">{row.label}</span>
            <span className={`text-center font-medium ${
              row.better ? "text-green-600 dark:text-green-400" : "text-foreground"
            }`}>
      ...
      <p className="text-xs text-muted-foreground mt-3">
        ⬆ — улучшение по сравнению с предыдущей моделью
      </p>
    </section>
  );
}
